...
const applyExp = (pokemon: Pokemon, expGain: number): Pokemon => {
  ...
  return {
    ...
    exp: newExp,
    level: newLevel,
    hp: Math.min(Math.max(pokemon.hp, 0) + (newStats.hp - pokemon.maxHp), newStats.hp),
    maxHp: newStats.hp,
  ...
  persist(
    (set, get) => {
        ...
        set((state) => {
          ...
          return updatePokemon(id, x => {
            ...
            return {
              ...
              stats: newStats,
              maxHp: newStats.hp,
              hp: Math.min(newStats.hp, Math.max(x.hp, 0) + hpDiff)
            };
          });
        });
      },
      ...
      changeStats: (delta, targetId) => {
        set((state) => {
          ...
          return updatePokemon(targetId, p => ({
            ...p,
            hp: delta.hp !== undefined ? Math.min(p.maxHp, Math.max(0, p.hp + delta.hp)) : p.hp,
      ...
      gainRandomEv: (targetId, amount = 1) => {
        set((state) => {
          ...
          const res = updatePokemon(targetId, (x) => ({
            ...
            stats: newStats,
            baseStats,
            hp: Math.min(Math.max(x.hp, 0) + (newStats.hp - x.maxHp), newStats.hp),
            maxHp: newStats.hp,
          }));
      ...
      restoreTeamHunger: () => {
        let ok = false;
        set((state) => {
          const cost = state.battleTeam.length * 50;
          if (state.coins < cost) return state;
          ok = true;
          const newParty = state.party.map(p => {
      ...
      useIvItem: (itemId, targetId) => {
        ...
        set((state) => {
          ...
          // Update maxHp if HP changed
          const hpDiff = newStats.hp - (pokemon.stats?.hp ?? 0);
          const newHp = pokemon.hp + hpDiff;
          ...
          const updateFn = (p: Pokemon) => {
            if (p.id === id) {
              ...
              return {
                ...
                hp: Math.min(Math.max(p.hp, 0) + (newStats.hp - p.maxHp), newStats.hp),                maxHp: newStats.hp,
                baseStats: newBaseStats,
                stats: newStats,
                ability: abilityInfo.ability,
                isHiddenAbility: abilityInfo.isHidden,
        ...
        set(() =>
          updatePokemon(id, (p) => {
            ...
            return {
              ...p,
              isShiny,
              hp: Math.min(Math.max(p.hp, 0) + (newStats.hp - p.maxHp), newStats.hp),
              maxHp: newStats.hp,
              baseStats,
              stats: newStats,
            };
      ...
      test_addLevel: () => {
        set((state) => {
          ...
          return updatePokemon(targetId, (p) => applyExp(p, 0)); // force re-eval with current level
        });
      },
