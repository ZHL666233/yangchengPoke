...
export default function Home() {
  ...
  useEffect(() => {
    const updateScale = () => {
      if (wrapperRef.current && contentRef.current) {
        ...
        if (contentH > wrapperH && wrapperH > 0) {
          setScale(Math.floor((wrapperH / contentH) * 100) / 100);
        } else {
          setScale(1);
        }
      }
  ...
  return (
    <>
    <div ref={wrapperRef} className="h-full w-full bg-slate-50 flex items-center justify-center overflow-hidden">
      <div
        ref={contentRef}
        className="w-full max-w-sm flex flex-col items-center px-6 py-8 relative"
      ...
      >
        ...
        <motion.button
          ...
          onClick={handleAdopt}
          className="mt-10 w-full bg-emerald-500 text-white font-black text-xl py-5 rounded-3xl shadow-xl shadow-emerald-200 hover:bg-emerald-600 disabled:opacity-50 disabled:shadow-none transition-all z-10"
        >
          就决定是你了！
        </motion.button>
        ...
        {!selectedStarter && (
          <motion.button
            ...
            className="mt-4 w-full bg-indigo-50 text-indigo-700 font-bold py-3 rounded-2xl border border-indigo-200 hover:bg-indigo-100 transition-colors z-10 text-sm"
          >
            查看新手引导
          </motion.button>
        )}
    ...
    {/* Guide Modal - outside scaled area */}
    {showGuide && (() => {
