...
import { BaseStats, SPECIES_INFO, calculateStats, generateRandomNature, emptyEvs, getBaseStats, Skill, Pokemon, ABILITY_INFO, getInitialSkills, generateAbility, STAT_NAMES, isPerfectIv, MAX_HUNGER, MAX_HAPPINESS, getAllLearnedSkills } from '@/types';
...
export default function Dungeon() {
  ...
  const {
    party, box, battleTeam, coins, pokedex,
    ...
    claimDungeonFloorReward, healTeam, switchPokemon, setSkills
  } = useGameStore();
    ...
    <div className="h-full w-full bg-gradient-to-b from-indigo-50 via-white to-white relative overflow-hidden flex flex-col">
      ...
      {!inBattle ? (
        ...
        <div className="relative flex-1 flex flex-col z-10 overflow-hidden">
          {/* Battle background */}
          <div className="absolute inset-0 pointer-events-none bg-gradient-to-b from-slate-300/40 via-slate-200/30 to-slate-100/20" />
            ...
            {wildPkmn && (
              <div className="flex justify-end w-full relative">
                <div className="flex flex-col items-end">
                  <div
                    className="bg-white/80 rounded-lg px-2.5 py-1 shadow-sm border border-slate-200 mb-1 w-40 relative cursor-pointer hover:border-blue-300 hover:shadow-md transition-all"
                  ...
                  <div className={wildAnim}>
                    <PokemonImage speciesId={wildPkmn.speciesId} alt="wild" className="w-28 h-28 object-contain" />
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="flex justify-start w-full -mt-1">
              <div className="flex flex-col items-start">
                <div className="bg-white/80 rounded-lg px-2.5 py-1 shadow-sm border border-slate-200 mt-1 w-40 relative">
                  ...
                  <div className="h-1 w-full bg-slate-200 rounded-full overflow-hidden">
                    <motion.div
                      className={`h-full ${playerHp / activePokemon.maxHp < 0.3 ? 'bg-red-500' : 'bg-emerald-500'}`}
            ...
            {showTeamSelect && (
              ...
              >
                <motion.div
                  ...
                  className="w-full bg-slate-900 rounded-t-3xl p-5 text-white max-h-[60vh] overflow-y-auto"
                >
                  <div className="flex justify-between items-center mb-4">
                    <span className="font-black text-lg">选择替换宝可梦</span>
                    <button onClick={() => setShowTeamSelect(false)} className="text-slate-400 hover:text-white">
                      <X className="w-5 h-5" />
                    </button>
                  </div>
                  <div className="grid grid-cols-3 gap-3">
                    {battlePokemons.map((p, idx) => {
                      const isActive = idx === activePartyIdx;
                      const isDead = p.hp <= 0;
                      return (
                        <button
                          key={p.id}
                          ...
                          onClick={() => {
                            switchPokemon(p.id, true);
                            setActivePartyIdx(idx);
                            setPlayerHp(p.hp);
                            setShowTeamSelect(false);
                          }}
                          className={`rounded-xl p-3 border flex flex-col items-center gap-1 transition-colors ${
                            isActive ? 'bg-emerald-800 border-emerald-600 cursor-default' :
                            isDead ? 'bg-slate-800 border-slate-700 opacity-40 cursor-not-allowed' :
                            'bg-slate-800 border-slate-700 hover:border-blue-400'
                          }`}
                        >
                          <PokemonImage speciesId={p.speciesId} alt={p.name} isShiny={p.isShiny} className="w-14 h-14 object-contain" />
                          ...
                          {isActive && <span className="text-[9px] font-black text-emerald-400">出战中</span>}
                          {isDead && <span className="text-[9px] font-black text-red-400">倒下</span>}
                        </button>
                      );
                    })}
                  </div>
