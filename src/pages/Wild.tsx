...
export default function Wild() {
  ...
  const [playerHpRaw, setPlayerHpRaw] = useState(0);
  // Wrapper to track every HP change
  const setPlayerHp = (hp: number) => {
    console.log('[setPlayerHp] called with:', hp, 'prev:', playerHpRaw, new Error().stack?.split('\n').slice(1,4).join(' | '));
    setPlayerHpRaw(hp);
  };
  ...
  const handleRestoreHunger = () => {
    ...
    const needCount = battleTeam.filter(id => {
  ...
  const closeBattle = () => {
    console.log('[closeBattle] called, battlePokemons HP:', battlePokemons.map(p => ({ name: p.name, hp: p.hp })));
    setInBattle(false);
    setWildPkmn(null);
    setShowBag(false);
  ...
  const switchPokemon = (index: number) => {
    ...
    console.log('[switchPokemon] clicked index:', index, 'newActivePokemon:', newActivePokemon?.name, newActivePokemon?.hp, 'party HP:', party.map(p => ({name: p.name, hp: p.hp})));
    if (!newActivePokemon || newActivePokemon.hp <= 0) {
      console.log('[switchPokemon] BLOCKED - hp <= 0 or not found');
      return;
    }
    ...
    if (currentSpeed >= wildSpeed) {
      ...
      const newCurrentHp = Math.max(0, playerHpRaw - wildDamage);
  ...
  const explore = (isBossEncounter: boolean = false) => {
    ...
    // Use LIVE store HP to prevent exploring with dead team (stale closure guard)
    const liveBattle = battleTeam.map(id => party.find(p => p.id === id) || box.find(p => p.id === id)).filter(Boolean) as Pokemon[];
    const aliveIdx = liveBattle.findIndex(p => p.hp > 0);
    if (aliveIdx === -1) {
      setEventResult({
        type: 'trap',
        message: '体力不支',
    ...
    window.setTimeout(() => {
      ...
      if (type === 'coins') {
        ...
        const wildNature = generateRandomNature();
        const baseStats = getBaseStats(speciesId);
        const stats = calculateStats(baseStats, ivs, evs, wildNature, level, isShiny);
  ...
  const attack = (skill?: Skill) => {
    if (!wildPkmn || !activePokemon || wildPkmn.hp <= 0 || playerHpRaw <= 0 || isActionDisabled) return;
    
    let pDmg = 0;
    let isCrit = false;
    let wildHp = wildPkmn.hp;
    let myHp = playerHpRaw;
    let typeMulti = 1; // 属性克制倍率，默认1
    ...
    if (skill) {
      ...
      // STAB：技能属性与使用者属性相同时 ×1.5
      const playerTypes = getPokemonTypes(activePokemon.speciesId);
      const stab = playerTypes.includes(skill.type) ? 1.5 : 1;
      // 属性克制：技能属性对野生宝可梦属性的倍率
      const wildTypes = getPokemonTypes(wildPkmn.speciesId);
      typeMulti = getTypeEffectiveness(skill.type, wildTypes).multiplier;
    ...
    function executePlayerAttack() {
      ...
      const typeHint = typeMulti >= 2 ? '效果拔群！' : typeMulti === 0 ? '没有效果…' : typeMulti < 1 ? '效果不佳…' : '';
      setBattleLog(`你使用了 ${skill ? skill.name : '攻击'}！造成了 ${pDmg} 伤害${isCrit ? '(击中要害)' : ''}。${typeHint}`);
    ...
    function handleFaint(faintedId: string) {
      ...
      setTimeout(() => {
        // IMPORTANT: React state batching means party may still have stale HP values here.
        // We KNOW faintedId has hp=0 (we just set it). Skip that ID explicitly.
        // For other Pokemon, use live party HP values.
        const liveBattlePokemons = battleTeam.map(id => party.find(p => p.id === id) || box.find(p => p.id === id)).filter(Boolean) as Pokemon[];

        // Find next alive, explicitly excluding the fainted one (even if party shows old HP)
        const nextAliveIdx = liveBattlePokemons.findIndex((p, idx) => idx > activeTeamIndex && p.hp > 0 && p.id !== faintedId);
        if (nextAliveIdx !== -1) {
          setActiveTeamIndex(nextAliveIdx);
          setPlayerHp(liveBattlePokemons[nextAliveIdx].hp);
          setBattleLog(`去吧！${liveBattlePokemons[nextAliveIdx].name}！`);
          setIsActionDisabled(false);
        } else {
          // Also check from start, excluding faintedId
          const anyAliveIdx = liveBattlePokemons.findIndex(p => p.hp > 0 && p.id !== faintedId);
          if (anyAliveIdx !== -1) {
            setActiveTeamIndex(anyAliveIdx);
            setPlayerHp(liveBattlePokemons[anyAliveIdx].hp);
            setBattleLog(`去吧！${liveBattlePokemons[anyAliveIdx].name}！`);
            setIsActionDisabled(false);
          } else {
            setBattleLog('队伍全军覆没！');
            setTimeout(() => {
  ...
  const runAway = () => {
    ...
    if (Math.random() < escapeChance) {
      ...
      const newPlayerHp = Math.max(0, playerHpRaw - wDmg);
      ...
      if (newPlayerHp <= 0) {
          setTimeout(() => {
            // Use LIVE store HP values to avoid stale closure bug
            const liveBattlePokemons = battleTeam.map(id => party.find(p => p.id === id) || box.find(p => p.id === id)).filter(Boolean) as Pokemon[];
            const nextAliveIdx = liveBattlePokemons.findIndex((p, idx) => idx > activeTeamIndex && p.hp > 0);
            if (nextAliveIdx !== -1) {
              setActiveTeamIndex(nextAliveIdx);
              setPlayerHp(liveBattlePokemons[nextAliveIdx].hp);
              setBattleLog(`去吧！${liveBattlePokemons[nextAliveIdx].name}！`);
              ...
              const anyAliveIdx = liveBattlePokemons.findIndex(p => p.hp > 0);
              if (anyAliveIdx !== -1) {
                setActiveTeamIndex(anyAliveIdx);
                setPlayerHp(liveBattlePokemons[anyAliveIdx].hp);
                setBattleLog(`去吧！${liveBattlePokemons[anyAliveIdx].name}！`);
  ...
  const throwPokeball = (ballId: ItemId) => {
    ...
    setTimeout(() => {
      ...
      if (Math.random() < finalProb || ballId === 'masterball') {
        ...
        const nature = wildPkmn.nature || generateRandomNature();
        const ivs = wildPkmn.ivs || (wildPkmn.isShiny ? { hp: 31, atk: 31, def: 31, spa: 31, spd: 31, spe: 31 } : generateRandomIvs());
        const evs = wildPkmn.evs || emptyEvs();
      ...
      {!inBattle ? (
        <div className="relative px-6 py-6 flex flex-col gap-5 flex-1 overflow-y-auto">
          ...
          <div className="rounded-3xl border border-emerald-200 bg-white/80 shadow-sm p-5">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                {' '}HP: {Math.floor(playerHpRaw)}/{activePokemon.maxHp}</div>
                <div className="h-1.5 w-full bg-slate-200 rounded-full overflow-hidden mt-1">
                  <motion.div 
                    className={`h-full ${playerHpRaw / activePokemon.maxHp < 0.3 ? 'bg-red-500' : 'bg-emerald-400'}`}
                    animate={{ width: `${(playerHpRaw / activePokemon.maxHp) * 100}%` }}
                  />
                </div>
          ...
          <motion.button
            ...
            disabled={rolling || playerHpRaw <= 0 || activePokemon.hunger < 5 || activePokemon.happiness < 3}
            className={[
          ...
          >
            ...
            {rolling ? '探索中...' : activePokemon.hunger < 5 || activePokemon.happiness < 3 ? '状态不足' : playerHpRaw <= 0 ? '需要休息' : '探索四周'}
          </motion.button>
            ...
            <div className="flex justify-start w-full -mt-1">
              <div className="flex flex-col items-start">
                <div className="bg-white/80 rounded-lg px-2.5 py-1 shadow-sm border border-slate-200 mt-1 w-40 relative">
                  ...
                  <div className="text-right text-[9px] font-bold text-slate-400 mt-0.5">
                    {Math.max(0, Math.floor(playerHpRaw))}/{activePokemon.maxHp}
                  </div>
                </div>
              ...
              <div className="space-y-2">
                ...
                {activePokemon.skills.map((s, idx) => (
                  <button
                    ...
                    disabled={s.pp <= 0 || playerHpRaw <= 0 || isActionDisabled}
